...
export default function Formulacao() {
  ...
  const load = () => {
    Promise.all([
      ...
      setFormulacoes(rf.data)
      setIngredientes(ri.data)
  ...
  const handleSaveForm = async () => {
    ...
    try {
      ...
      if (editingForm) {
        await api.put(`/api/formulacoes/${editingForm.id}`, payload)
      } else {
        await api.post('/api/formulacoes', payload)
      }
  ...
  const handleDeleteForm = async (f) => {
    if (!window.confirm(`Excluir formulação "${f.nome}"?`)) return
  ...
  // --- Ingredientes ---
  const openCreateIng = () => {
    setEditingIng(null); setIngForm(emptyIng); setIngError(''); setShowIngModal(true)
  }
  const openEditIng = (i) => {
    setEditingIng(i)
  ...
  const handleSaveIng = async () => {
    setIngError('')
    if (!ingForm.nome) { setIngError('Nome é obrigatório'); return }
    setSavingIng(true)
    try {
      if (editingIng) {
        await api.put(`/api/ingredientes/${editingIng.id}`, ingForm)
      } else {
        await api.post('/api/ingredientes', ingForm)
      }
  ...
  if (loading) return <Layout title="Formulação de Ração"><div className="loading"><div className="spinner" /> Carregando...</div></Layout>

  return (
    <Layout title="Formulação de Ração">
      <div className="page-header">
        <div><h1>🌾 Formulação de Ração</h1><p>Gerencie ingredientes e receitas de ração</p></div>
        <div style={{ display: 'flex', gap: 8 }}>
          {canEdit() && tab === 'formulacoes' && (
            <button className="btn btn-primary" onClick={openCreateForm}>+ Nova Formulação</button>
          )}
        ...
        <div>
          {formulacoes.length === 0 ? (
            <div className="table-container">
              <div className="table-empty"><span className="empty-icon">🧪</span>Nenhuma formulação cadastrada</div>
            </div>
          ) : formulacoes.map(f => (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                ...
                {canEdit() && (
                  <div className="actions">
                    <button className="btn btn-outline btn-sm" onClick={() => openEditForm(f)}>✏️ Editar</button>
            ...
            <tbody>
              {ingredientes.length === 0 ? (
                ...
                <tr key={i.id}>
                  ...
                  <td>{i.unidade}</td>
                  <td>
                    <span className={`badge ${i.estoque_kg > 0 ? 'badge-green' : 'badge-red'}`}>
                      {i.estoque_kg} {i.unidade}
                    </span>
                  </td>
      ...
      {showFormModal && (
        <Modal
          title={editingForm ? `Editar: ${editingForm.nome}` : 'Nova Formulação de Ração'}
        ...
        >
          ...
          <div className="form-grid">
            <div className="form-group">
              ...
              <input value={form.nome} onChange={e => setF('nome', e.target.value)} placeholder="Ex: Ração Creche 20kg" />
            </div>
            <div className="form-group">
              <label>Fase</label>
            ...
            {itens.map((item, idx) => {
              ...
              return (
                <div key={idx} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: 8, marginBottom: 8, alignItems: 'end' }}>
                  <div>
                    <label style={{ fontSize: 12 }}>Ingrediente</label>
                  ...
                  <div>
                    <label style={{ fontSize: 12 }}>% na ração {custo > 0 && <span style={{ color: '#198754' }}>= R${custo.toFixed(4)}/kg</span>}</label>
                    <input
                      type="number" min="0" max="100" step="0.01"
                      value={item.percentagem}
                      onChange={e => setItem(idx, 'percentagem', e.target.value)}
                      placeholder="0.00"
                    />
                  </div>
                  <button className="btn btn-danger btn-sm" type="button" onClick={() => removeItem(idx)} style={{ marginBottom: 0 }}>✕</button>
                </div>
              )
            })}
      ...
      {showIngModal && (
        <Modal
          title={editingIng ? `Editar: ${editingIng.nome}` : 'Novo Ingrediente'}
        ...
        >
          ...
          <div className="form-grid">
            <div className="form-group span-2">
              <label>Nome *</label>
              <input value={ingForm.nome} onChange={e => setI('nome', e.target.value)} placeholder="Ex: Milho grão, Farelo de soja..." />
            </div>
            <div className="form-group">
              <label>Unidade</label>
